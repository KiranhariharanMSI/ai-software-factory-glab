...
8. **Never modify how VCS credentials are handled** (the `gh`/`glab`/backend token
   read path) or `guard.py`'s protected-path enforcement.
...
**Infrastructure:** none present in this repo (no Dockerfile, `deploy/`, `infra/`).
  If any is added later, it is protected from the commit that adds it.
**Secrets:** `.env*`, `*secret*`, `*credential*`, `*.pem`
**Security invariants:** the VCS credential read path (`gh`/`glab`/backend token
  handling) and `guard.py`'s protected-path enforcement itself.
